...
namespace UnitTestEx.MSTest.Test
{
    ...
    public class PersonFunctionTest
    {
        ...
        public void WithBody()
        {
            using var test = FunctionTester.Create<Startup>();
            test.HttpTrigger<PersonFunction>()
                .Run(f => f.Run(test.CreateJsonHttpRequest(HttpMethod.Get, new { name = "Jane" }, "person"), test.Logger))
        ...
        public void BadRequest1()
        {
            using var test = FunctionTester.Create<Startup>();
            test.HttpTrigger<PersonFunction>()
                .Run(f => f.Run(test.CreateJsonHttpRequest(HttpMethod.Post, new { name = "Brian" }, "person"), test.Logger))
        ...
        public void ValidJson()
        {
            using var test = FunctionTester.Create<Startup>();
            test.HttpTrigger<PersonFunction>()
                .Run(f => f.Run(test.CreateJsonHttpRequest(HttpMethod.Get, new { name = "Rachel" }, "person"), test.Logger))
