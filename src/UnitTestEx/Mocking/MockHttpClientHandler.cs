...
namespace UnitTestEx.Mocking
{
    ...
    public class MockHttpClientHandler : DelegatingHandler
    {
        ...
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            logger.LogDebug($"Sending HTTP request {request.Method} {request.RequestUri} {LogContent(request.Content)}");
            ...
            logger.LogDebug($"Received HTTP response {response.StatusCode} ({(int)response.StatusCode}) {LogContent(response.Content)}");
            return response;
        }
