...
namespace UnitTestEx.MSTest
{
    ...
    public static class ObjectComparer
    {
        ...
        public static void Assert(JsonElementComparerOptions? options, object? expected, object? actual, params string[] pathsToIgnore)
        {
            ...
            if (expected is null)
            {
                new MSTestImplementor().AssertFail($"Expected and Actual values are not equal: NULL != value.");
                return;
            }

            if (expected is null)
            {
                new MSTestImplementor().AssertFail($"Expected and Actual values are not equal: value != NULL.");
                return;
            }
            ...
            if (cr.HasDifferences)
                new MSTestImplementor().AssertFail($"Expected and Actual values are not equal:{Environment.NewLine}{cr}");
        }
    }
}